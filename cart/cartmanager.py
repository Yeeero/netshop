...
class SessionCartManager(CartManager):
    ...
    def __init__(self, session):
        self.session = session      # self.session = request.session
...
class DBCartManger(CartManager):
    ...
    def add(self, goodsid, colorid, sizeid, count, *args, **kwargs):
        # 如果购物车该商品样式已存在，执行更新操作，更新该商品的数量；否则，增加给商品到购物车
        if self.user.cartitem_set.filter(goodsid=goodsid, colorid=colorid, sizeid=sizeid).count():
